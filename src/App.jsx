    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8">
            ...
            ) : (<div className="h-full flex flex-col items-center justify-center text-slate-300"><CheckCircle size={80} className="mb-4 opacity-10"/><p className="font-black uppercase tracking-widest text-sm">Select student to record data</p></div>)}
        </div>
      </div>
      {preview && (
      ...
      <div className="flex-1 p-10 overflow-y-auto">
        {tab === 'setup' ? (
          <div className="max-w-2xl bg-white p-10 rounded-3xl shadow-sm border mx-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-2xl border-2 border-dashed">
                {school.logo_url ? <img src={school.logo_url} className="h-16 w-16 object-contain" alt="logo" onError={(e)=>e.target.style.display='none'}/> : <School size={32} className="text-slate-300"/>}
          ...
          <div className="bg-white rounded-2xl border shadow-sm">
            ...
            {students.map(s => (<div key={s.id} onClick={async () => {const { data: co } = await supabase.from('comments').select('*').eq('student_id', s.id).maybeSingle(); setSelectedStudent(s); setCommentData(co || {}); }} className="p-6 border-b hover:bg-indigo-50 cursor-pointer flex justify-between items-center transition"><div><p className="font-bold">{s.name}</p><p className="text-[10px] uppercase font-black text-slate-400">{s.classes?.name}</p></div><span className="text-indigo-600 font-bold text-xs uppercase">Process Review →</span></div>))}
          </div>
        )}
      </div>
      {selectedStudent && (
...
const ParentPortal = ({ onBack }) => {
  ...
  if (data) return (<div className="fixed inset-0 z-50 bg-white flex flex-col"><button onClick={()=>setData(null)} className="p-6 bg-slate-900 text-white font-black flex items-center gap-3 text-xs uppercase tracking-widest">← Back to search</button><PDFViewer className="flex-1 border-none"><ResultPDF school={data.schools} student={data} results={data.results} comments={data.comments[0]} type="full" /></PDFViewer></div>);

  return (<div className="min-h-screen bg-indigo-950 flex items-center justify-center p-4"><div className="bg-white p-12 rounded-[60px] shadow-2xl max-w-md w-full text-center"><GraduationCap size={70} className="mx-auto text-indigo-600 mb-6 p-4 bg-indigo-50 rounded-3xl"/><h2 className="text-3xl font-black mb-10 text-slate-800 tracking-tight uppercase tracking-[0.2em]">Parent Portal</h2><input className="w-full border-2 p-5 rounded-[30px] mb-4 text-center font-black tracking-widest text-lg uppercase outline-none focus:border-indigo-600 shadow-sm" placeholder="ADM-XXXX" onChange={(e)=>setId(e.target.value)} /><button onClick={lookup} className="w-full bg-indigo-600 text-white py-6 rounded-[30px] font-black shadow-2xl shadow-indigo-200 transition active:scale-95 uppercase text-xs tracking-widest">{loading ? <Loader2 className="animate-spin mx-auto"/> : 'Access Transcript'}</button><button onClick={onBack} className="mt-8 text-slate-400 block w-full text-xs font-black uppercase hover:underline">Employee Login</button></div></div>);
};
