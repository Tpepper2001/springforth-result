      ...
      {preview && (
        <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col p-4">
          <div className="bg-white p-4 flex justify-between rounded-t-3xl items-center shadow-xl">
            ...
            <PDFDownloadLink document={<ResultPDF school={school} student={selectedStudent} results={results} comments={commentData} type={preview}/>} fileName="cavendish_result.pdf">
              <button className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg">DOWNLOAD PDF</button>
            </PDFDownloadLink>
          </div>
          <PDFViewer className="flex-1 rounded-b-3xl border-none"><ResultPDF school={school} student={selectedStudent} results={students.find(x=>x.id===selectedStudent.id)?.results || []} comments={commentData} type={preview}/></PDFViewer>
        </div>
      )}
    </div>
  );
